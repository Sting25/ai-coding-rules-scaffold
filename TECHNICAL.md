...
| _generated by `install.sh`, no template_                                | `.gitignore` (rules appended; file created if missing)          | Adds `*.scaffold-bak` and `*.scaffold-bak.*` so upgrade backups never get swept into a commit. Written before any other file, on every run, including one that goes on to refuse a write (for example a symlinked `.githooks`/`.github`/`.claude`/`.cursor` and exit non-zero): this edit lands even when the rest of the install does not. |
